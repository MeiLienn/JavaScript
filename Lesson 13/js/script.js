...
//Модальное окно для кнопок "Узнать подробнее"
	...
	persons.addEventListener('change', function(){
		...
		if (restDays.value == '' || personsSum < 0 || daysSum < 0 || !(Number.isInteger(personsSum)) || !(Number.isInteger(daysSum)) ){
			totalValue.innerHTML = 0;
		}else {
	...
	restDays.addEventListener('change', function(){
		daysSum = +this.value;
		total = (daysSum * personsSum)* 4000;
		if (persons.value == '' || daysSum < 0 || personsSum < 0 || !(Number.isInteger(daysSum)) || !(Number.isInteger(personsSum)) ){
			totalValue.innerHTML = 0;
		}else {
			totalValue.innerHTML = total;
		}
